...
ssh-add secrets/lsb-pkey-rsa

git clone git@github.com:graphistry/deploy.git

cd deploy

sed 's/52.8.250.251/52.9.34.130/' < lsbmono > lsbremotelaunch
...
ssh -o StrictHostKeyChecking=no -i tools/ansible_id_rsa.pem ubuntu@52.9.34.130 whoami

ansible-playbook site.yml -i lsbremotelaunch
