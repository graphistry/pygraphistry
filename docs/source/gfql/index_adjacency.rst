...
.. warning::
   **The measured figures that used to appear here have been withdrawn.** They
   ...
   harness that produces
   :doc:`the boards on the performance page <performance>` — committed per-slot
   artifacts, recorded commit/host/perf-lock/reps, and result rows validated
   against the competitor before any ratio is published.
