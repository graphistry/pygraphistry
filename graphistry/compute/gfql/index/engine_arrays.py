...
def select_by_ids(df: DataFrameT, col: str, ids: ArrayLike, engine: Engine) -> DataFrameT:
    ...
    if engine in (Engine.POLARS, Engine.POLARS_GPU):
        ...
        # Semi-join (not Expr.is_in(Series), which polars >= 1.28.0 deprecates as
        # ambiguous — pola-rs/polars#22149; see lazy/engine/polars/membership.py) — vectorized AND preserves the left (df) row order, which
