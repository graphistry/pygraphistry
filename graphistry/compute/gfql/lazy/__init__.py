...
Lazy engines build a deferred plan and execute it once on a chosen **execution target**
(CPU / GPU now; remote backends later). This package is the shared framework — the target
abstraction + collect-once helpers — that each lazy backend (``lazy/engine/polars``, future
``lazy/engine/duckdb``) plugs into. Per-backend *lowering* lives under
``lazy/engine/<backend>/`` and is NOT shared (polars builds ``pl.LazyFrame``, DuckDB would
build relations).

Design (from the GPU benchmark + architecture review): ``engine='polars'`` and
``engine='polars-gpu'`` are ONE lazy polars engine with two ``.collect()`` targets, not two
engines. The win requires ONE plan collected ONCE (transfer-once, fused) — per-op eager
collect loses on GPU (repeated H2D) — so backends accumulate a plan and call
:func:`collect` / :func:`collect_all` at the materialization boundary, on the active target.
"""
...
# Two knobs, each resolved LIVE at call time (not frozen at import), in order:
# (1) Python override (set_*, None = unset) > (2) env var > (3) default.
# NOTE: overrides are PROCESS-GLOBAL module state (unlike _TARGET, a ContextVar that
# target_mode sets transiently) — a startup-time global default, not per-request/thread;
# a concurrent set_cpu_streaming() affects all in-flight GFQL queries in the process.
...
def gpu_executor() -> GpuExecutor:
    ...
    'in-memory' is fast + stable for results fitting device memory (the GFQL regime — see
    :func:`_engine_for`); 'streaming' is the opt-in escape hatch for larger-than-device results
    (in-memory would OOM), slower/less stable on small work. ONLY these two values are accepted
    (``GPU_EXECUTORS``); a size-aware 'auto' is a possible future addition but NOT selectable
    today (``set_gpu_executor('auto')`` raises).
...
# A GFQL call() may run a Plottable-method ANALYTIC (umap/hypergraph/compute_cugraph/
# compute_igraph/layout_*/collapse/...) with NO native polars impl, ever — it runs eagerly on
# pandas/cuDF. Under engine='polars'|'polars-gpu' this knob decides:
#   'auto' (default): BRIDGE — run off-engine on pandas (polars) / cuDF (polars-gpu), coerce
#          back to polars losslessly (Arrow), warn once per (process, function).
#   'strict': DECLINE with NotImplementedError — benchmark integrity (no hidden modality
#          switch) or a hard memory ceiling.
# DELIBERATELY distinct from CHAIN traversal/filter/row ops, which stay parity-or-NIE (a bridge
# there would hide a missing impl + cheat a benchmark). Mirrors the
# GRAPHISTRY_CUDF_SAME_PATH_MODE auto/strict precedent.
...
# Public surface. Explicit (no dedicated types.py): the quasi-public types
# (ExecutionTarget/GpuExecutor/CallMode + tuples) are few and co-located with their
# validators/accessors; a types module is deferred until a 2nd lazy backend (duckdb)
# needs to share ExecutionTarget (mirrors the lazy-restructure defer).
__all__ = [
...
def _engine_for(target: ExecutionTarget) -> "Optional[pl.GPUEngine]":
    ...
    GPU uses the cudf-polars IN-MEMORY executor, not the default streaming `engine="gpu"`
    (`DefaultSingletonEngine`): GFQL results fit device memory — the in-memory regime — and it
    is FASTER (semijoin 1.33×, antijoin 2.58×, unique 1.49× @10M) and STABLE (streaming spiked
    bimodally to ~1 s on the same semijoin; in-memory holds ~30 ms). ``raise_on_fail=True`` is
    the NO-CHEATING contract for the GPU target: any non-GPU-executable plan node RAISES —
    never silently run on CPU and reported as GPU. (raise_on_fail=False looked honest — the
    fallback stays in Polars, not a pandas bridge — but it makes polars-gpu indistinguishable
    from polars whenever the plan isn't fully GPU-capable, mislabeling CPU work as GPU; hard
    raise forces the truth: polars-gpu is GPU-or-error, use polars for CPU.) Larger-than-device
    inputs OOM rather than stream — acceptable (in-scope gfql graphs fit); revisit if that changes."""
    if target == ExecutionTarget.GPU:
        import polars as pl
        # RAPIDS/cudf_polars-not-installed is checked at the chain dispatch (pre-coercion) so
        # engine='polars-gpu' gets a clean install error there; here we only build the engine —
        # a genuinely non-GPU-capable plan is reported via _gpu_raise. Executor defaults to
        # in-memory; GFQL_POLARS_GPU_EXECUTOR=streaming opts into streaming for
        # larger-than-device results (see gpu_executor()).
...
def _gpu_raise(exc: Exception) -> "NotImplementedError":
    """Translate a cudf-polars GPU failure (raise_on_fail=True, non-GPU-executable plan node)
    into the NO-CHEATING error: a clear NotImplementedError chained from the polars error
    (not a cryptic ComputeError), pointing at the CPU engine."""
...
def collect_all(lfs: "List[pl.LazyFrame]") -> "List[pl.DataFrame]":
    """Collect several LazyFrames in ONE pass on the active target: common subplans (e.g. the
    edge table) materialize/transfer once — the whole point of going lazy on GPU. Falls back to
    per-frame collect if the installed polars lacks ``collect_all``."""
    import polars as pl
    eng = _engine_for(active_target())
