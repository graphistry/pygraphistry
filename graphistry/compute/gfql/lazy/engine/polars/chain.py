...
def _chain_traversal_polars(self: Plottable, ops, start_nodes: Optional[Any] = None) -> Plottable:
    ...
    if start_nodes is None and len(ops) == 3 and _fp_node(ops[0]) and _plain_edge(ops[1]) and _fp_node(ops[2]):
        ...
        if unconstrained or e1.direction in ("forward", "reverse"):
            ...
            # A node filter draws its ids FROM the node table, so a filtered side is already
            # closed; a synthesized node table is vacuously closed.
            closure_cols = ([col for col, filter_ids in filter_sides if filter_ids is None]
            ...
            if closure_cols:
                ...
                if unresolvable.item() > 0:
                    for endpoint_col in closure_cols:
