...
def _two_hop_equal_domain_dense_total(
    ...
    *,
    ...
    (in-degrees) plus one O(E) gather-sum -- no hash join, no group_by, no count
    join, and no second degree table: ``sum_b indeg(b) * outdeg(b)`` equals
    ``sum_e indeg(src(e))`` because each middle node contributes its in-degree
    once per out-edge.
    ...
    if 0 <= lo and hi + 1 <= table_budget:
        ...
        # buffer -- allocating two fresh shifted arrays cost ~3x the counting body
        # itself at 2.4M edges (same page-fault anatomy as above). The gather runs
