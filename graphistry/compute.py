...
class ComputeMixin(MIXIN_BASE):
    ...
    def hop(
        ...
        if not to_fixed_point and not isinstance(hops, int):
            raise ValueError(
                f"Must provide hops int when to_fixed_point is False, received: {hops}"
            )
        ...
        while True:
            ...
            if direction in ["forward", "undirected"]:
                hop_edges_forward = wave_front.merge(
                    edges_indexed[[g2._source, g2._destination, EDGE_ID]].rename(
                        columns={g2._source: g2._node}
                    ),
                    how="inner",
                    on=g2._node,
                )[[g2._destination, EDGE_ID]]
                new_node_ids_forward = (
                    hop_edges_forward[[g2._destination]]
                    .rename(columns={g2._destination: g2._node})
                    .drop_duplicates()
                )
            ...
            if direction in ["reverse", "undirected"]:
                hop_edges_reverse = wave_front.merge(
                    edges_indexed[[g2._destination, g2._source, EDGE_ID]].rename(
                ...
                new_node_ids_reverse = (
                    hop_edges_reverse[[g2._source]]
                    .rename(columns={g2._source: g2._node})
                    .drop_duplicates()
                )
            ...
            new_node_ids = pd.concat(
                []
                + (
                    [new_node_ids_forward] if new_node_ids_forward is not None else []
                )  # noqa: W503
                + (
                    [new_node_ids_reverse] if new_node_ids_reverse is not None else []
                ),  # noqa: W503
                ignore_index=True,
                sort=False,
            ).drop_duplicates()
            combined_node_ids = pd.concat(
                [matches_nodes, new_node_ids], ignore_index=True, sort=False
            ).drop_duplicates()

            matches_edges = pd.concat(
                [matches_edges]
                + (
                    [hop_edges_forward[[EDGE_ID]]]
                    if hop_edges_forward is not None
                    else []
                )  # noqa: W503
                + (
                    [hop_edges_reverse[[EDGE_ID]]]
