...
class ComputeMixin(MIXIN_BASE):
    ...
    def replace_nodes_with_edges(self, node_selector, type: str = ''):
        """
        return the nodes in g with edges given by  node_selector

        node_selector can be of type str, list(str), DataFrame or a series.

        If node_selector is a str or list(str), type defaults to g._destination

        If node_selector is a DataFrame or a Series, node id binding must be set
        and it overrides "type"
        """
        g = self
        ...
        if isinstance(node_selector, list) or isinstance(node_selector, str):
            node_selector = list(node_selector)

        if isinstance(node_selector, pandas.DataFrame):
            if node_selector.shape[1] > 1:
                raise ValueError('node_selector DataFrame is of wrong dimensions')
            if g._node:
                node_selector = node_selector[0].drop_duplicates().to_list()
                type = g._node
            else:
                raise ValueError('Must set node id binding, not just nodes; set via .bind() or .nodes()')
        elif isinstance(node_selector, pandas.Series):
            if g._node:
                node_selector = node_selector.drop_duplicates().to_list()
                type = g._node
            else:
                raise ValueError('Must set node id binding, not just nodes; set via .bind() or .nodes()')

        if type == g._source:
            select_col = g._destination
        elif (type == g._destination) or (type == ""):
            type = g._destination
            select_col = g._source
        else:
            raise ValueError('type must be either source or destination')

        if len(node_selector) == 0:
            return g

        nodes_to_reduce = g._edges[g._edges[type].isin(node_selector)][select_col].drop_duplicates().values
        if len(nodes_to_reduce) <= 1:
            raise ValueError('No nodes to reduce in the selected type')

        edges_to_retain = g._edges[~g._edges[type].isin(node_selector)]

        df = pandas.DataFrame(
            0,
            index=nodes_to_reduce, columns=nodes_to_reduce) \
            .where(numpy.tril(numpy.ones(nodes_to_reduce.shape)).astype(bool)) \
            .isna() \
            .stack() \
            .reset_index()
        df.columns = [g._source, g._destination, 'Values']
        g._edges = pandas.concat(
            [
                df[df['Values']].drop(['Values'], axis=1).assign(reduced_from=",".join(node_selector)),
                edges_to_retain
            ], ignore_index=True)
        return g
