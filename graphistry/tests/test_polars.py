...
# Module-level fixtures — only evaluated when polars is installed
if HAS_POLARS:
...
class TestPolarsInternals(NoAuthTestCase):
    ...
    def test_table_to_arrow_dataframe(self):
        result = CGFull()._table_to_arrow(EDGES_PL, memoize=False)
        self.assertIsInstance(result, pa.Table)
        self.assertFalse(result.schema.metadata)  # polars metadata must be stripped

    def test_table_to_arrow_lazyframe(self):
        result = CGFull()._table_to_arrow(EDGES_LAZY, memoize=False)
        self.assertIsInstance(result, pa.Table)
    ...
    def test_table_to_arrow_schema_metadata_stripped(self):
        ...
        result = CGFull()._table_to_arrow(NODES_PL, memoize=False)
        self.assertIsInstance(result, pa.Table)
        self.assertFalse(result.schema.metadata)
