...
triangleNodes = pandas.DataFrame(
...
class TestEtl2Metadata(NoAuthTestCase):
    def test_metadata_mandatory_fields(self, mock_etl2, mock_open):
        ...
        self.assertEqual(
            dataset["encodings"]["nodes"],
            {"pointTitle": {"attributes": [nid]}, "nodeId": {"attributes": [nid]}},
        )
        self.assertEqual(
            dataset["encodings"]["edges"],
            {"source": {"attributes": ["src"]}, "destination": {"attributes": ["dst"]}},
        )
    ...
    def test_metadata_double_name(self, mock_etl2, mock_open):
        ...
        graphistry.bind(source="src", destination="dst", node="id").plot(
            edges, triangleNodes
        )
        dataset = mock_etl2.call_args[0][0]
    ...
    def test_metadata_no_nan(self, mock_etl2, mock_open):
        ...
        edges["testInt"] = triangleNodes.a1.map(
            lambda x: numpy.nan if x % 2 == 1 else 0
        )
        edges["testFloat"] = triangleNodes.a1.map(
            lambda x: numpy.nan if x % 2 == 1 else 0.5
        )
        edges["testString"] = triangleNodes.a1.map(
            lambda x: numpy.nan if x % 2 == 1 else "foo"
        )
        edges["testBool"] = triangleNodes.a1.map(
            lambda x: numpy.nan if x % 2 == 1 else True
        )
    ...
    def test_metadata_no_nat(self, mock_etl2, mock_open):
        edges = triangleEdges.copy()
        edges["testDate"] = triangleNodes.a1.map(
            lambda x: None if x == 1 else datetime.datetime.now()
        )
