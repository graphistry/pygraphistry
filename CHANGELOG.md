...
| `graphistry.api_key(...)` or `graphistry.pygraphistry.PyGraphistry.api_key(key_id, key_secret)` | `graphistry.register(personal_key_id=..., personal_key_secret=...)` | `api_key()` was the legacy api=1 key setter, not the api=3 personal-key login helper. It is deprecated and returns `None`. |
