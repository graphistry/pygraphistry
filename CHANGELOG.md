...
* GFQL: native Polars single-node and directed scalar one-hop chains use resident node-property indexes to resolve seeds. Typed edges, destination filters, and named patterns now share the seeded reduction used by Cypher, with full-path table order and alias flags preserved (#2033).
