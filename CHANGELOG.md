...
### Fixed
- **GFQL on an NVRTC-less CUDA install no longer crashes (`engine='cudf'`)**: cudf imports and runs its precompiled ops fine on a host whose CUDA lacks NVRTC (`libnvrtc.so`), but cupy's kernel compiler — which the GFQL index/count kernels' `bincount`/`searchsorted` need — raises `RuntimeError` at first use, deep inside a query. A once-per-process probe in `array_namespace` now detects the missing compiler and falls back to numpy host arrays for the cudf engine: identical values, host-side compute, no crash. Pinned: forced-fallback backend + array types, probe caching.
