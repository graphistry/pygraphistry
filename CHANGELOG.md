...
- **GFQL / cudf**: `graph.gfql(query, engine="cudf")` no longer crashes with SIGSEGV (returncode 139) on RAPIDS 25.02. Root cause: `Series.apply(lambda)`, `Series.map(dict)`, and `Series.to_pandas()` on cudf Series all trigger numba JIT (via `numba_cuda.as_cuda_array`) which SIGSEGVs in RAPIDS 25.02. Fixed by introducing `safe_map_series()` in `Engine.py` that uses a merge-based lookup with `to_arrow()` for cudf (stays on GPU, avoids numba), and replacing all seven affected call sites in `gfql_unified.py`, `hop.py`, `chain.py`, `df_executor.py`, and `pipeline.py`. Validated on both RAPIDS 25.02 and 26.02. GPU performance lane restored (#977).
