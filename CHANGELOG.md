...
### Performance
- **GFQL fused two-star grouped count (graph-benchmark q5–q7 class) drops two provably redundant semi-joins and folds the grouped tail into the one collect.** The fused polars lane for `MATCH (p)-[r1]->(i), (p)-[r2]->(c) WHERE ... RETURN keys, count(p)` carried a LEFT-arm shared-domain semi-join that commutes with the per-src count into the INNER join that follows it (out-of-domain groups are dropped by the join, per-key counts untouched — the one consumer that needs the RESTRICTED counts, the empty-match all-left-counts==1 probe behind the openCypher n=0 row, rebuilds them WITH the semi-join on that branch only, pinned by a test whose out-of-domain person holds a count of 2), a RIGHT-arm second-leaf semi-join whose unique-keyed group-property lookup join keeps exactly the semi's row multiset (without group properties the semi stays), and an eager three-op group_by/sort/head tail that now rides the SAME single collect when group keys exist (same ops, same maintain_order, same null placement; grouped LIMIT 0 keeps the eager tail, pinned). Values byte-identical vs the pandas oracle and the forced-decline eager twin. On the receipted DGX candidate lanes (combined publication build `938f22851` = master `f875724ce` + the four graph-benchmark perf branches; pyg-bench `results/graphbench-board-{20k,100k}-cand-20260803`), the comparator's verdicts move: q7 `TIE` (polars 5.68 ms) → `WIN 1.49x` (3.47 ms) at 20k and `TIE` (9.88 ms) → `WIN 1.46x` (6.69 ms) at 100k; q5@100k `TIE` (13.68 ms) → `WIN 1.18x (WEAK: slot ranges overlap)` (11.35 ms); q6 widens `WIN 1.55x` → `WIN 1.96x` at 20k and `WIN 1.62x` → `WIN 1.89x` at 100k; q5@20k `WIN 1.20x (WEAK: slot ranges overlap)` → `WIN 1.29x` with no overlap.
