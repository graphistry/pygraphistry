...
- **GFQL Cypher numeric functions + `^` power operator + `toLower`/`toUpper` (openCypher/neo4j-standard)**: added the standard scalar functions `floor`, `ceil` (alias `ceiling`), `round(x)` / `round(x, precision)`, and `toLower` / `toUpper` (the idiomatic case-insensitive compare, `WHERE toLower(n.name) = 'bob'`), plus the exponentiation operator `^` (right-associative, binds tighter than `* / %`, returns a float — `10 ^ 5` → `100000.0`), across the Cypher `WHERE`/`RETURN` expression surface. Evaluated natively on pandas/cuDF and polars (differential-parity tested). Complements the already-supported `abs`/`sqrt`/`sign` and chained comparisons (`WHERE 1 < n.age < 65`). Two documented minor divergences: `round` uses the pandas/numpy default rounding (round-half-to-even) rather than neo4j's mode set (the `round(x, precision, mode)` 3-arg form is not yet supported); and a negative *literal* base like `-2 ^ 2` folds to `(-2) ^ 2 = 4` (the numeric lexer reads `-2` as a literal) whereas neo4j yields `-(2^2) = -4` — a column base `-n.x ^ 2` is unaffected. `LIKE`/`ILIKE`/`BETWEEN` remain intentionally unimplemented (not in Cypher or GQL).
- **GFQL Cypher numeric functions + `toLower`/`toUpper` (openCypher/neo4j-standard)**: added the standard scalar functions `floor`, `ceil` (alias `ceiling`), `round(x)` / `round(x, precision)`, and `toLower` / `toUpper` (the idiomatic case-insensitive compare, `WHERE toLower(n.name) = 'bob'`), across the Cypher `WHERE`/`RETURN` expression surface. Evaluated natively on pandas/cuDF and polars (differential-parity tested). Complements the already-supported `abs`/`sqrt`/`sign` and chained comparisons (`WHERE 1 < n.age < 65`). Minor documented divergence: `round` uses the pandas/numpy default rounding (round-half-to-even) rather than neo4j's mode set (the `round(x, precision, mode)` 3-arg form is not yet supported). The `^` exponentiation operator is deferred (the openCypher TCK treats `^` as left-associative and marks it reject-expected in the current corpus, so it needs a coordinated corpus + associativity change). `LIKE`/`ILIKE`/`BETWEEN` remain intentionally unimplemented (not in Cypher or GQL).
