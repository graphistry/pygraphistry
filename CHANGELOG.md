...
- **GFQL / Cypher**: Connected non-linear comma `MATCH` fanout now lowers through joined bindings rows in the local compiler, so star shapes like `MATCH (p)<-[:R]-(x), (x)-[:S]->(a), (x)-[:S]->(b)` can execute through downstream row stages instead of failing on the older single-linear-path normalizer (#1000).
