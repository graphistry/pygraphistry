...
### Tests
- **GFQL / Cypher**: Multi-row scalar prefix reentry — 9 new tests covering two-tag fanout, carried scalar visible in RETURN, empty prefix, single-row regression, bag-semantics duplicate-seed (explicit 4-row assertion), partial-hit (some prefix rows produce empty suffix), optional_reentry + multi-row guard raises, cuDF path (#1047).
- **GFQL / Cypher**: Parser and execution tests for bounded zero-min variable-length relationships (`*0..N`, typed, multi-type, zero-hop inclusion, `*0` exact still rejected); negative hop bounds (`*-1`, `*-5..10`, `*1..-3`) raise syntax error (#983).
