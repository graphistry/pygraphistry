...
- **CI / release pip cooldown enforcement**: `publish-pypi.yml` now upgrades to `pip>=26.0`, computes a UTC cutoff timestamp from the 6-day policy, and passes `--uploaded-prior-to <cutoff>` explicitly on pip install steps used by release builds (`-e .[build]`, `cyclonedx-bom`) so pip-side cooldown behavior is explicit and version-verified.
