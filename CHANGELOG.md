...
- **CI / release provenance baseline**: `publish-pypi.yml` now adds CycloneDX SBOM generation (`dist/sbom-cyclonedx.json`), GitHub build-provenance attestation for built distributions (`dist/*`), and release-evidence artifact upload for auditability. PyPI publish attestations are now enabled for both TestPyPI and PyPI paths.
- **CI / publish dry-run controls**: `publish-pypi.yml` `workflow_dispatch` now accepts `publish_mode` (`dryrun`, `testpypi`, `full`) so maintainers can rehearse release evidence generation and/or TestPyPI publish without requiring a full PyPI publish. `full` mode remains restricted to `master`.
