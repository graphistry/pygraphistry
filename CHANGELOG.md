...
### Changed
- **GFQL / Cypher compiler policy MVP (#1454)**: Added a direct-only `postcompile` policy hook for local Cypher string queries. The hook receives a stable `compiler_summary` payload with query hash, alias bindings, projection items, group keys, and aggregate summaries for provenance/benchmark policy code without exposing private compiler objects or parsing exception strings. Runtime policy shortcuts remain unchanged: broad `post` handlers do not implicitly run `postcompile`.
