...
### Performance
- **HOLDBACK: the dense-domain two-hop count kernel's O(E) passes host-thread on the numpy backend.** Above a fixed edge threshold the remaining bincount and the gather-sum (both release the GIL) run chunked across a small process-lifetime thread pool, merging O(domain) partial tables; below it the exact single-threaded lane is kept, and cupy is untouched (already device-parallel). The pool holds no query-derived state and sits deliberately outside `gfql_clear_caches()`. Held back pending the owner's call on library-owned thread pools (oversubscription/fork interactions) and on whether the thread-free round-4 lever suffices. Pins: threaded-vs-plain equality on uneven chunk boundaries; kernel-level threaded/unthreaded/oracle triple.
