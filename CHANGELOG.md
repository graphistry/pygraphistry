...
### Infrastructure
- **The test extra now pins `tzdata`**, so named-timezone tests do not depend on the host shipping a complete system tzdata. Slim images and minimal distros ship a partial `/usr/share/zoneinfo` — one with `America/` and `Europe/` but no `US/` makes `test_align_mixed_tz_converts_only_mixed_datetime_pairs` fail with `ZoneInfoNotFoundError: 'US/Eastern'`, which reads like a pandas behaviour change and is not one. Toggling only `tzdata` in an otherwise identical environment moves the five temporal test modules from 1 failed / 1402 passed to 1403 passed.
...
- GFQL `searchAny` / `search_nodes` / `search_edges` now search **datetime** columns when the term is numeric, matching what the viz inspector displays rather than `repr`: a timestamp renders as the inspector's moment format (`Jan 5 2024, 3:04:05 am UTC`), so `"2024"` finds rows in that year. Datetimes previously declined. The zone is a `temporal_tz` parameter (default UTC) because the inspector renders in the viewer's zone, which a server cannot know — and the zone is not cosmetic, since it decides which day a timestamp lands on and therefore which rows match. Verified byte-identical to real `moment-timezone` on 3000 timestamps across 12 zones, including both DST directions, half- and quarter-hour offsets, and zones whose abbreviation changes seasonally.
  - Searching a datetime column means rendering it, which a numeric-term search now pays for on every datetime column present: on a 300k-row frame one datetime column takes a `searchAny` from 77ms to 629ms, about three times what a float column added when #2089 gated floats in the same way. A term that is not numeric does not gate datetime columns in and pays nothing, which is pinned by spying on the renderer rather than by timing.
  - pandas and polars render every zone and answer identical rows; polars matches pandas on 2968/3000 fuzz cases, the residue being `Africa/Casablanca`, where polars' bundled timezone database and Python's disagree about its future DST. cuDF matches at UTC (0/5000 mismatches on a GB10) and declines any other zone rather than mislabel it: cudf 26.02.01's `strftime` ignores `tz_convert`, answering `UTC` for `%Z` whatever the zone and reading the underlying UTC instant for `%p`, so an `Asia/Kolkata` column would render the right hour beside the wrong meridiem.
...
- **`circle_layout()` entry guards (#1968)**: an edge-only graph no longer crashes with `TypeError: object of type 'NoneType' has no len()` — nodes are materialized before they are counted, matching every other layout on the surface. The no-argument form on a graph without `x`/`y` positions now raises a `ValueError` naming the documented precondition (pass `bounding_box=` or have positions) instead of a bare `KeyError: 'x'`. A partition-keyed `bounding_box` frame with duplicate `partition_key` values now raises a `ValueError` naming the duplicated keys instead of a pandas `IntCastingNaNError` from the row-multiplying merge.
- **Engine coercion can still name the engines when it degrades**: `ensure_engine_match` returns the original graph rather than crash a user's workflow, and its handler reports which engines it saw — but those names are resolved inside the same `try`, so a failure early enough left them unset. The handler papered over this with `'nodes_engine' in locals()`; the names are now simply bound before the `try`, and the degradation contract is pinned by tests for a failure both before and after they are resolved.
- **`adjacency_membership_keys` no longer depends on its direction argument being one of three strings to have bound anything**: `oi` and `ii` were each assigned only under their own direction check, so an unrecognized direction reached `assert oi is not None` with nothing bound and raised `UnboundLocalError` instead of the assertion. Both are initialized up front; the existing asserts now say what they were written to say.

- **`from_igraph` determines its node ID column without a bookkeeping flag**: the branch that un-swizzles integer vertex IDs tracked whether it had found a matching column in a separate `found` boolean, so the column name was bound inside the loop and read outside it. Binding `node_id_col = None` up front and testing that instead removes the flag and the possibly-unbound read. Behaviour is unchanged; round-trips through igraph produce byte-identical edge frames for named vertices, integer vids and a bare attribute-less graph.

- **Two feature-pipeline functions raised `UnboundLocalError` on reachable inputs**: `transform()` bound `X` only inside its `nodes`/`edges` branches, while `y` and `T` beside it were pre-initialized, so any other `kind` — and `kind` is a plain `str` that `FastEncoder` does not validate — crashed at the first `X.empty`; and `get_numeric_transformers()` returns `ndf_` unconditionally but assigned it only when `ndf is not None`, so passing `None` crashed, where the sibling `y_ = y` had the same shape and worked. Both now bind on every route, matching the idiom already used beside them.

- **Three layout paths read a local that an earlier branch may never have bound**, so the failure mode was `UnboundLocalError` rather than the intended behaviour. `GraphBase.__init__` read the component `s` in its connectivity check even when no edge ever assigned it (reachable when vertices arrive with a component already set); it now initializes to `None`, which the existing check already treats as unconnected. `tangents()` appended the leaked loop variable after the loop to repeat the final tangent; it now appends by value. `mercator_layout` bound `cupy` only inside its import guard and gated on a separate `use_cupy` flag; the flag is folded into `cp is not None`. No behaviour changes: `tangents` was verified identical on every input, and the mercator cupy branch was verified equal to the numpy branch to 1e-9 on a GB10 (cudf 26.02.01, cupy 13.6.0) including equator and ±89° cases.
