...
- **Remote `format='csv'` no longer silently rewrites result values (#1958)**: `gfql_remote`/`chain_remote`/`python_remote_*` decoded csv responses with a bare `read_csv`, so pandas/cudf re-inferred dtypes from text: `'007'` came back as `7.0`, `'08'` as `8.0`, and `'NA'`/`''`/`'null'` as `NaN`. Worse, node `id` landed as `float64` while the edge endpoints stayed `int64`, so the returned graph could not join to itself and every downstream local op on it was wrong or empty. csv is untyped on the wire and the response carries no dtype manifest, so the client cannot reconstruct the server's schema; csv is now declined with a `ValueError` naming the limitation and pointing at `format='parquet'` (the default, and faithful). Callers who still want csv pass the new `df_import_args` reader kwargs to take explicit control, e.g. `df_import_args={'dtype': {'id': str}, 'keep_default_na': False, 'na_values': []}`, which round-trips values and keeps the node/edge join coherent. The decline happens before the request is sent. `format='parquet'` and `format='json'` are unchanged.
