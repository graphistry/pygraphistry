...
- **CI / workflow scanners**: Added `workflow-security.yml` with two checks for workflow changes: `actionlint` (with custom runner-label config in `.github/actionlint.yaml`) and `zizmor` high-severity gating using `.github/zizmor.yml`. This phase explicitly defers SHA-pin enforcement (`unpinned-uses`) to future unscheduled work.
