...
### Changed
- **GFQL temporal text helper split (#1498)**: Split temporal value parsing, duration parsing/folding, truncation folding, and AST rewrite helpers into `graphistry.compute.gfql.temporal` modules while keeping the existing `graphistry.compute.gfql.temporal_text` import surface stable. No temporal semantics changed.
