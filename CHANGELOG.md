...
### Performance
- **The dense-domain two-hop `count(*)` kernel elides its interval shift (graph-benchmark q8@100k, round 2).** Line-level profiling overturned round 1's report that the residual was "memory-bound bincount": on the 100k graph-benchmark (2.42M FOLLOWS edges, Person ids 1..100000) the 26.3 ms local kernel split as 16.2 ms materializing the two shifted id arrays (two fresh ~19 MB allocations per call, page-fault bound in-context), 3.2 ms in the bounds-proof chains, and only 6.2 ms in the bincounts. Three changes inside the proof-gated kernel, values identical by construction: when `lo >= 0` and `hi+1` fits the SAME table budget the domain guard already enforces, the raw arrays are counted with tables of size `hi+1` (the bounds proof guarantees nothing lands below `lo`, and the `[0, lo)` prefix is provably all-zero); distant or negative intervals shift through ONE reused scratch buffer instead of two fresh E-length arrays (mixed src/dst dtypes keep the plain shift); and the polars bounds proof fuses its six min/max/null_count reductions into one parallel select. Honest transfer note: the local ~2x on the counting body did NOT transfer to the DGX board host — the shift cost there was local page-fault behavior — so on the receipted DGX candidate lane the q8@100k polars cell moves 15.94 ms (round-1 kernel) → 14.72 ms and the comparator verdict stays `LOSE 1.52x` (vs `LOSE 2.44x` before the round-1 kernel; lanes `results/graphbench-board-{20k,100k}-cand-20260803`, combined publication build `938f22851`). Kept for the correctness-neutral simplification plus the small real gain; round 3 targets the warmed-Kuzu goalpost (~9.7 ms). Lane pins via an array-namespace spy (elision lane never allocates; distant-interval lane routes both columns through one scratch buffer; mixed-dtype lane keeps the plain shift), both sides of the table-budget boundary, and all existing round-1 value-parity and decline pins unchanged.
