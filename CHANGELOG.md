...
- **Release docs / metadata**: Updated publish instructions to pull `master` in fast-forward-only mode (`git pull --ff-only origin master`), push only the intended release tag (`git push origin X.Y.Z`) instead of `--tags`, clarified manual publish dispatch as maintainer-only recovery on `master`, and normalized legacy `pypi.python.org` links in `README.md`/`setup.py` to `pypi.org`.
