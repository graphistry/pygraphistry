...
### Changed
- **GFQL private compatibility shim cleanup (#1520)**: Removed obsolete private GFQL compatibility import shims for row dispatch, row pipeline, row ordering, order-expression helpers, and the historical temporal-text helper facade after auditing that repo callers now use the canonical owner modules. Also removed tests that only asserted retired private/import compatibility surfaces; runtime behavior is unchanged.
