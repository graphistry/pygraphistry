...
- **Cypher OLAP fast paths silently ran on CPU under `engine='polars-gpu'` (#1824)**: the fast paths execute before the chain route, which is where the GPU execution target used to be established, so their `LazyFrame.collect()` calls ran on the default CPU engine while being reported as polars-gpu results. The dispatch now sets the execution target around the fast paths and every collect goes through the target-honoring `lazy.collect`/`collect_all` (GPU-or-error: a plan the GPU cannot execute raises, recorded as an honest decline, and the chain route answers under its own GPU contract). A source pin forbids bare collects in the fast-path module; CPU engines are pinned unchanged.
- **LazyFrame node input crashed undirected single-hop Cypher on polars (#1740)**: the undirected backward threading semi-joins the user's node frame against a hop output, and polars joins do not mix eagerness — lazy nodes met an eager wavefront (`TypeError: expected 'other' to be a 'LazyFrame'`). The join now aligns eagerness; all four lazy/eager frame combinations are pinned to pandas parity.
- **Two-hop `count(*)` could return a STALE answer after an in-place frame mutation (#1825)**: the equal-domain degree counts were memoized by `setattr` onto the caller's `Plottable` keyed by `id()` — stable across in-place mutation and reusable after GC, the exact BLOCKER-1 pattern the same file forbids. The memo is removed rather than re-keyed: within one execution it never hit (one call per query), and CROSS-call reuse belongs to the fingerprint-validated index layer (`gfql_index_all`'s degree facts), which invalidates on mutation and is faster than the memo was. Pinned with a string-id fixture on purpose — dense-integer-id graphs are served by the dense kernel before the degree-count path, so the issue's own int-id repro reads as a false "fixed". Repeat-vs-one-shot parity is pinned as an identity. (Benchmark note: this removes the warm-Plottable advantage the unindexed q8 cell had; the published typed-index cell is registry-served and unaffected.)
