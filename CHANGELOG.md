...
### Performance
- **Per-type column-stat facts become opt-in, and build in one pass**: the per-type facts added for typed graphs are not free — measured on a 420k-node / 2.8M-edge graph they added ~104 ms to a ~262 ms `gfql_index_all` (+39%), while only typed count shapes can spend them (~40 such queries to break even). Two changes follow. The edge role now aggregates BOTH endpoint bindings in a SINGLE grouped pass instead of one pass per endpoint, cutting the edge-side cost roughly in half (+52.0 ms → +28.0 ms; total +103.8 ms → +75.1 ms). And building them from a bound `GraphSchema` is now behind `col_stats_by_type=` on `gfql_index_col_stats()` / `gfql_index_all()`, defaulting False — binding a schema no longer silently changes index cost (with a schema bound the opt-in measures +9.2%, and it scales with label count since `label__X` costs one pass per label). Explicit `node_type_column=` / `edge_type_column=` requests are unaffected: asked for by name, still built, still raising when unusable. Values are unchanged in every arm — a regression where the shared pass fingerprinted facts across all aggregated columns (making each look stale on lookup) was caught by the engagement pins and fixed by fingerprinting per column. Pins: single-pass vs per-column equivalence, all-or-nothing decline across columns, and the opt-in gate on both entry points.
