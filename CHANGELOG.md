...
- **GFQL / Cypher residual unplanned-chain fallback hardening (#1468)**: Cypher logical-planner deferrals now carry structured `logical_plan_defer_code` metadata, and runtime chain fallback is limited to explicitly approved residual buckets (non-top-level `OPTIONAL MATCH`, multiple-`MATCH`/shortestPath lanes, and scalar projection alias shapes). Unclassified or unknown `logical_plan is None` compiled programs now fail fast before chain execution.
...
### Fixed
- **GFQL / Cypher ordinary sequential multiple MATCH native route (#1477)**: Connected non-optional sequential `MATCH ... MATCH ...` clauses now route through the planned connected-match join payload instead of the residual `multiple_match_stages` unplanned-chain fallback. shortestPath/exotic multi-`MATCH` shapes remain explicitly deferred. Added compile/runtime cutover coverage proving ordinary sequential MATCH bypasses `_execute_compiled_query_chain_non_union()`.
