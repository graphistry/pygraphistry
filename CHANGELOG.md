...
- **GFQL / cudf**: `graph.gfql(query, engine="cudf")` no longer crashes with SIGSEGV (returncode 139) on RAPIDS 25.02. Root cause: `Series.apply(lambda)` and `Series.map(dict)` on cudf Series trigger numba JIT which SIGSEGVs in RAPIDS 25.02. Fixed by introducing `safe_map_series()` in `Engine.py` that bridges through pandas for cudf Series, and replacing all seven affected call sites in `gfql_unified.py`, `hop.py`, `chain.py`, `df_executor.py`, and `pipeline.py`. GPU performance lane restored (#977).
