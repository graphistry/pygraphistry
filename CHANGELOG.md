...
### Internal
- **GFQL / AST edge JSON construction cleanup (#1512)**: Consolidated duplicated edge `from_json()` field extraction and validation logic in `graphistry.compute.ast` behind private helpers while preserving the existing `ASTEdge*` classes, aliases, constructors, and wire payload behavior.
