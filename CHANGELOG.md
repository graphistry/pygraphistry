...
- **CI / pip cooldown enforcement cleanup**: direct `pip install` callsites in workflows now pass explicit upload-time cutoffs via `--uploaded-prior-to` (derived from `PIP_COOLDOWN_DAYS=6`) for GPU, DGL, Spark, UMAP/AI torch, and `workflow-security` `zizmor` installs. Removed ambiguous `PIP_EXCLUDE_NEWER` workflow env usage in favor of explicit per-command pip filtering.
