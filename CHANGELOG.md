...
### Added
- **GFQL / IR compiler**: Added `graphistry/compute/gfql/ir/verifier.py` with `verify(plan: LogicalPlan) -> list[CompilerError]`. Tree-walks the operator DAG and checks five structural invariants: op_id uniqueness, dangling child references, predicate scope (non-empty expression on PatternMatch/Filter/IndexScan), output schema validity (all columns are LogicalType instances), and optional-arm nullability (PatternMatch optional=True requires non-empty arm_id and nullable ScalarType outputs). Traversal is cycle-safe via DFS ancestor tracking. `verify` is exported from `ir/__init__.py`. 60 focused positive/negative tests; one documented xfail for ListType element_type deep recursion (future PR) (#1127).
