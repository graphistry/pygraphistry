...
def get_data(model='botnet', nrows=10000):
    ...
    X = None
    if model == 'botnet':
        x_feats = ['Dur', 'Proto', 'SrcAddr', 'Sport', 'DstAddr', 'Dir',
       'Dport', 'State', 'sTos', 'dTos', 'TotPkts', 'TotBytes', 'SrcBytes'] # removed 'StartTime',
        df = get_botnet_dataframe(nrows=nrows)
        y = df.Label
        X = df[x_feats]
        print(f'{model} is a labeled dataset!')
    if model == 'netflow':
        x_feats = ['epochtime', 'duration', 'srcDevice', 'dstDevice', 'protocol',
       'srcPort', 'dstPort', 'srcPackets', 'dstPackets', 'srcBytes',
       'dstBytes'] # same as cols
        df = get_netflow_dataframe(nrows=nrows)
        X = df
        print(f"{model} is an unsupervised dataset")
    if model == 'host':
        x_feats = ['epochtime', 'eventID', 'logHost', 'userName', 'domainName', 'logonID',
...
def objective(trial, model='botnet'):
    df, X, y, sup_vec, sup_label = get_data(model) #ToDo move this outside as it takes a long time....
    
    # 2. Suggest values for the hyperparameters using a trial object.
    classifier_name = trial.suggest_categorical('classifier', ['SVC', 'RandomForest', 'HistGradient'])
    if classifier_name == 'SVC':
         svc_c = trial.suggest_float('svc_c', 1e-10, 1e10, log=True)
         classifier_obj = sklearn.svm.SVC(C=svc_c, gamma='auto')
    elif classifier_name == 'HistGradient':
        rf_max_depth = trial.suggest_int('rf_max_depth', 2, 32, log=True)
        classifier_obj = sklearn.ensemble.HistGradientBoostingRegressor(max_depth=rf_max_depth, n_estimators=10)
    else:
        rf_max_depth = trial.suggest_int('rf_max_depth', 2, 32, log=True)
        classifier_obj = sklearn.ensemble.RandomForestRegressor(max_depth=rf_max_depth, n_estimators=10)
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_enc, y_enc, train_size=0.9, shuffle=False)
    ...
    score = sklearn.model_selection.cross_val_score(model, X_train, y_train, n_jobs=-1, cv=3)
    ...
    effective_targets = [target_names[k] for k in y_enc.values.argmax(1)] # not ideal, but gets us there
    
    
    plot_df = pd.concat(
        [pd.DataFrame({'class': effective_targets}), pd.DataFrame(xys, columns=['x', 'y'])],
        axis=1
    )
    plot_df.head()
