...
umap_kwargs_probs = {'n_components': 2,
               'metric': 'hellinger',
               'n_neighbors': 15,
               }

umap_kwargs_euclidean = {'n_components': 2,
               'metric': 'euclidean',
               'n_neighbors': 7,
               }
...
class baseUmap():
    ...
    def _check_target_is_one_dimensional(self, y):
        ...
        if y.shape[1]<=1:
            ...
            logger.warning(f'Ignoring target column of shape {y.shape} as it is not one dimensional')
            return None
    ...
    def _edge_influence(self):
        if hasattr(self, 'reducer'):
            ...
            self.weighted_adjacency, self.umap_entity_to_index = pandas_to_sparse_adjacency(self.weighted_edges_df, src, dst, weight_col)
        else:
