...
def get_vectorizer(name):
    ...
    if name == config.SKLEARN:
        #TODO
        return
    logging.warning(f'Vectorizer name must be one of [{config.DIRTY_CAT}, {config.SKLEARN}, ..]')
    return
...
class BaseDGLGraphFromPandas():
    def __init__(self, ndf, edf, src, dst, node_column, node_target=None, edge_target=None, weight_col=None,
                 use_node_label_as_feature=False, vectorizer=process_dirty_dataframes,  device='cpu'):
        """
    ...
    def _convert_edgeDF_to_DGL(self):
        ...
        self.graph, self.adjacency, self.entity_to_index = \
            pandas2dgl(self.edf, self.src, self.dst, weight_col=self.weight_col, device=self.device)
    ...
    def _check_nodes_lineup_with_edges(self):
        ...
        logger.info(f'{len(nodes)} entities from column {self.node}\n with {len(unique_nodes)} unique entities')
        if len(nodes) != len(unique_nodes): # why would this be so?
            logger.warning(f'Nodes DataFrame has duplicate entries for column {self.node}')
        ...
        if res.sum() != len(nodes):
            logger.warning(f'Some Edges connect to Nodes not explicitly mentioned in nodes DataFrame (ndf)')
        if len(self.entity_to_index)>len(nodes):
            logger.warning(f'There are more entities in edges DataFrame (edf) than in nodes DataFrame (ndf)')
    ...
    def _node_featurization(self):
        ...
        if not self.use_node_label_as_feature:
            logger.info(f'Dropping {self.node} from DataFrame so not to include it as an explicit feature, '
    ...
    def _edge_featurization(self):
        ...
        edata, edge_encoders = featurization(self.edf, self.edge_target, self.vectorizer)
        self.edata = edata
