...
def get_host_dataframe(nrows=100000):
    df = pd.read_csv('pygraphistry/data/host_events.csv', nrows=nrows, index_col=0)
    return df


def get_netflow_dataframe(nrows=100000):
    df = pd.read_csv('pygraphistry/data/netflow_events.csv', nrows=nrows, index_col=0)
    return df

def get_botnet_dataframe(nrows=100000):
    df = pd.read_csv('pygraphistry/data/malware_capture_bot.csv', nrows=nrows, index_col=0)
