...
def resolve_runs_dir(runs_dir_arg: Optional[str]) -> Path:
...
def generate_issue_residual_report(
    runs_dir_arg: Optional[str],
    issue_ref: str = "graphistry/pygraphistry#880",
    include_all_runs: bool = False,
    include_child_issue_templates: bool = True,
) -> str:
    runs_dir = resolve_runs_dir(runs_dir_arg)
    lanes = load_residual_lanes(runs_dir)
    lanes = filter_issue_residuals(lanes, issue_ref=issue_ref)
    if not include_all_runs:
        lanes = latest_per_lane_key(lanes)
    return render_markdown_report(
        lanes=lanes,
        issue_ref=issue_ref,
        include_child_issue_templates=include_child_issue_templates,
    )
