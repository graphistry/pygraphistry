...
for profile_def in "${PROFILE_DEFS[@]}"; do
    ...
    for ver in "${VERSIONS[@]}"; do
        # Skip versions outside [min_python, max_python]
        if [ -n "$min_python" ] && ! python3 -c "import sys; sys.exit(0 if tuple(int(x) for x in '$ver'.split('.')) >= tuple(int(x) for x in '$min_python'.split('.')) else 1)" 2>/dev/null; then
            echo "--- SKIP ${name}-py${ver} (requires >=${min_python}) ---"
            continue
        fi
        if [ -n "$max_python" ] && ! python3 -c "import sys; sys.exit(0 if tuple(int(x) for x in '$ver'.split('.')) <= tuple(int(x) for x in '$max_python'.split('.')) else 1)" 2>/dev/null; then
            echo "--- SKIP ${name}-py${ver} (requires <=${max_python}) ---"
            continue
        fi
